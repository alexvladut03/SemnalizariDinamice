...
import { updateAttribute } from "@/utils/actions/attributes/update-attribute";
import { TrashIcon } from "lucide-react";

const EditAttribute = ({ attribute }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [textInputs, setTextInputs] = useState(attribute.values);
  ...
  const { execute, result, isExecuting } = useAction(updateAttribute, {
    onSuccess: ({ data }) => {
      setIsOpen(false);
      reset();
      toast({
        variant: "default",
        title: "Succes",
        description: `Atributul ${data.attribute.name} a fost editat cu succes!`,
        duration: 3000,
      });
    },
    onError: () => {
      toast({
        variant: "destructive",
        title: "Eroare",
        description: "A intervenit o eroare la editarea atributului.",
        duration: 3000,
      });
    },
  });
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form
            onSubmit={(e) => {
              ...
              execute({ name, slug, values, id: attribute.id });
            }}
            className="space-y-8"
          >
            <FormField
            ...
            {result.validationErrors?.values && (
              <p>{result.validationErrors?.values._errors}</p>
            )}
